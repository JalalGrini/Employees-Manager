...
<div class="page-header animate__animated animate__fadeIn">
    ...
    <p>View attendance records by department and date.</p>
</div>

<div class="filter-container">
    <form action="{{ route('attendancess') }}" method="GET" class="filter-form">
        <select name="department" id="department" class="filter-select">
            <option value="">-- Select a department --</option>
            <option value="Software Development" {{ request('department') == 'Software Development' ? 'selected' : '' }}>Software Development</option>
            <option value="Artificial Intelligence" {{ request('department') == 'Artificial Intelligence' ? 'selected' : '' }}>Artificial Intelligence</option>
            <option value="Infrastructure & Networks" {{ request('department') == 'Infrastructure & Networks' ? 'selected' : '' }}>Infrastructure & Networks</option>
            <option value="IT Security" {{ request('department') == 'IT Security' ? 'selected' : '' }}>IT Security</option>
            <option value="Cloud & DevOps" {{ request('department') == 'Cloud & DevOps' ? 'selected' : '' }}>Cloud & DevOps</option>
            <option value="Design & User Experience" {{ request('department') == 'Design & User Experience' ? 'selected' : '' }}>Design & User Experience</option>
            <option value="Human Resources" {{ request('department') == 'Human Resources' ? 'selected' : '' }}>Human Resources</option>
            <option value="Digital Marketing" {{ request('department') == 'Digital Marketing' ? 'selected' : '' }}>Digital Marketing</option>
            <option value="Finance & Accounting" {{ request('department') == 'Finance & Accounting' ? 'selected' : '' }}>Finance & Accounting</option>
            <option value="Sales & Business" {{ request('department') == 'Sales & Business' ? 'selected' : '' }}>Sales & Business</option>
            <option value="Customer Support" {{ request('department') == 'Customer Support' ? 'selected' : '' }}>Customer Support</option>
            <option value="Strategy & Innovation" {{ request('department') == 'Strategy & Innovation' ? 'selected' : '' }}>Strategy & Innovation</option>
        </select>
        <input type="date" name="date" id="date" class="filter-date" value="{{ request('date') }}">
        <button type="submit" class="filter-button">Filter</button>
        <a href="{{ route('attendancess') }}" class="filter-button clear-button">Clear</a>
    </form>
</div>
